...
export default function EnhancedDataTable({ data = [], columns = [] , onSelectInputText }) {
  ...
  const row = filteredData[rowIndex]; // Use filteredData instead of data
  if (onSelectInputText) {
    onSelectInputText(isSameRow ? null : row);
  }
};
 ...
 <tr
  ...
  className={`
    ...
    ${row.text_type === 'prompt' ? styles.typePrompt : ''}
    ${row.text_type === 'response' ? styles.typeResponse : ''}
  `}
